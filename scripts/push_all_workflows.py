...
#
# For each version of each workflow, which is usually a git release tag, like "vX.Y.Z" for "WORKFLOW_NAME-vX.Y.Z"
# 1. Build a Docker image for the Dockerfile for WORKFLOW_NAME at the given tag(s)
# 2. Push Docker image to EMR
# 3. Upload all WDL files for WORKFLOW_NAME at the given tag to the czid-workflows S3 bucket
#
# TODO: Build based on Git TAG instead of the main branch.
#       This used to be done in the workflows-infra task, and maybe should be done so again.
#       Or, even better, have a GitHub Action for each Workflow, with optional Version.
#       But still might be ideal if ran from the workflow infra project, as it used to
...
workflow_to_versions_dict = {
    ...
    # TODO: phylotree-ng doesn't match any versions in CZI, and is unused. So why build it at all?
    "phylotree-ng": [
        "v6.11.0"
    ],
    "short-read-mngs": [
...
def push_all_workflows_to_ecr(profile: str):
    print("build_args_dict =", build_args_dict)
...
def push_workflow_to_s3(profile: str, workflow_name: str, workflow_version: str, workflows_bucket: str):
    ...
    print("remote_filenames =", sorted(remote_filenames))
    ...
    for filename in local_filenames:
        ...
        #     ]
        # )
        s3_cp_result = run_cmd(
            [
                "aws",
                "s3",
                "cp",
                f"{local_filepath}",
                f"{s3_dir_uri}/{filename}",
                "--profile",
                profile
            ]
        )
            ...
            if not os.path.isfile(local_filepath_zipped):
                zip_result = run_cmd(
                    [
                        ...
                        local_filepath,
                        "--output",
                        local_filepath_zipped,
                        "--path",
                        local_dir
                    ]
                )
                print("zip_result = ", zip_result)
                s3_cp_zip_result = run_cmd(
                    [
                        "aws",
                        "s3",
                        "cp",
                        f"{local_filepath}.zip",
                        f"{s3_dir_uri}/{filename}.zip",
                        "--profile",
                        profile
                    ]
                )
...
# TODO: Only do this for Prod / idseq-prod:
#      [[ $(aws iam list-account-aliases | jq -r '.AccountAliases[0]') == "idseq-prod" ]]; then
...
def main():
    ...
    parser.add_argument(
        ...
        "--environment",
        type=str,
        choices=['sandbox', 'dev', 'staging', 'prod'],
