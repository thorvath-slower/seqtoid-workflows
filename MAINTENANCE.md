...
| A5 | Pinned reference-data S3 paths (dated snapshots) | `short-read-mngs/non_host_alignment.wdl` / `postprocess.wdl` / `experimental.wdl` (`s3://czid-public-references/.../2021-01-22/…`); `amr/run.wdl` (`card/2023-05-22/…`); `consensus-genome/run.wdl`; `short-read-mngs/host_filter_defaults.yml` | Hardcoded bucket + dated reference DB versions; pipeline-science decision, no updater | Update the date/path by hand when reference data is regenerated; coordinate with index-generation. **DO NOT bulk-rename the `czid-public-references` bucket in these WDL defaults to `seqtoid-public-references`** without first mirroring the objects AND making the destination bucket anonymously-readable. The live app path overrides the bucket at dispatch (`S3_DATABASE_BUCKET`), so it is unaffected either way, but the credential-less benchmark CI (`.github/workflows/short-read-mngs-viral-benchmarks.yml`) blackholes the IMDS endpoint and configures no AWS creds, so miniwdl reads these defaults ANONYMOUSLY. `s3://czid-public-references` is anonymously public; `s3://seqtoid-public-references` currently returns AccessDenied to anonymous callers (verified 2026-07-20), so a naive repoint breaks the benchmark. Prerequisite tracked under the platform-overhaul static-resource-migration ticket (P2). |
